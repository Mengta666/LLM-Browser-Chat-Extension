...
- _schedule_memory_write:后台线程抽取写入(复用 write_after_task)
...
def _build_memory_system(query: str) -> Optional[str]:
    """检索长期记忆 → 拼装成一条 system 注入文本。无记忆/异常返回 None。

    分层(对齐 MemGPT):
    - core 层(persona+preference):常驻全量注入(量小、都重要,不过闸门)。
    - episodic 层:与 query 相关才注入(过双相关性闸门,防 Lost-in-the-Middle 噪声)。
    复用 agent/memory service 门面,全程降级。
    """
    try:
    ...
    # episodic:按需(过相关性闸门,空则不拼)
    if query:
        try:
            episodic = memory_service.recall_episodic(query)
            if episodic:
...
def _inject_memory(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """把记忆注入为一条 system,插到 messages 最前。无记忆则原样返回。"""
    query = _extract_last_user_text(messages)
    mem_system = _build_memory_system(query)
    if not mem_system:
        return messages
...
def _schedule_memory_write(user_text: str, assistant_text: str, chat_id: str = "") -> None:
    ...
    def _worker():
        try:
            from agent.memory import service as memory_service
            result = memory_service.write_chat_memory(user_text, assistant_text)
...
def chat_completions(item: ChatRequest):
    ...
    messages = _inject_memory(item.messages)
