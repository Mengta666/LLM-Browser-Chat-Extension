...
HISTORY_GUIDANCE = (
    ...
    '最多回查4次，总返回预算4000 tokens；长消息按 next 继续读取，未读部分不得声称已经看过。'
)
HISTORY_TOOLS = [
...
class TurnHistoryBudget:
    ...
    def execute(self, name, args, *, max_tokens):
        ...
        payload = {'kind': 'session_history', 'upto_seq': self.upto_seq,
                   ...
                   'messages': [], 'has_more': more, 'next': None}
        for index, row in enumerate(rows):
            entry = {**row, 'history_id': f'H{row["seq"]}', 'truncated': row['content_offset'] + len(row['content']) < row['content_length']}
            ...
            while low < high:
                mid = (low + high + 1) // 2
                entry.update(content=original[:mid], truncated=row['content_offset'] + mid < row['content_length'])
                payload['next'] = {'start_seq': row['seq'], 'offset': row['content_offset'] + mid}
            ...
            if low == 0 and original:
                ...
                payload['next'] = {'start_seq': row['seq'], 'offset': row['content_offset']}
                break
            entry.update(content=original[:low], truncated=row['content_offset'] + low < row['content_length'])
            payload['next'] = None
            if not searching and entry['truncated']:
                payload['next'] = {'start_seq': row['seq'], 'offset': row['content_offset'] + low}
                payload['has_more'] = True
                break
            if index == len(rows) - 1 and not searching and row['seq'] < args['end_seq']:
                payload['next'] = {'start_seq': row['seq'] + 1, 'offset': 0} if more else None
        text = json.dumps(payload, ensure_ascii=False)
