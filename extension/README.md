...
扩展提供普通聊天、图片附件、历史会话、知识库管理及浏览器自动化，必须连接本项目 FastAPI 后端，不支持直接连接第三方模型接口。上游模型由后端调用。
...
4. 打开扩展侧边栏，进入设置页。
5. “Browser Agent 后端地址”默认是 `http://127.0.0.1:8000/v1`，远端部署时改为实际后端地址；模型填上游实际可用的模型名。本机 / 内网后端允许前端 Key 留空。
...
- **普通聊天**：关闭自动化，发送文本或图片；支持 Markdown、公式和流式响应。前端没有固定 8000 字限制，由后端检查完整请求的 Token 预算；当前输入本身超预算时仍会拒绝。具体图片 / 工具能力由模型服务决定。
- **图片附件**：Chat 支持文件选择、图片粘贴和拖拽；需要截图时使用系统截图工具，再粘贴或选择图片文件。
- **浏览器自动化**：启用自动化并输入任务。扩展负责观察、执行动作和回传结果，后端逐步决策；需要保持扩展流程运行。
- **历史会话**：使用本项目后端保存的历史及恢复协议。后端不可用或会话协议不兼容时明确报错，不切换为前端拼接历史。
...
- `activeTab` / `scripting`：页面读取及自动化页面操作。
