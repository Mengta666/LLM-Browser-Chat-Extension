...
1. **core**(关于用户的稳定事实,常驻记忆):长期稳定、跨对话有效的用户身份与偏好。
...
- 抽出时把"新状态"作为 content,不必强行合并新旧(mem0 的 additive 风格):
  ...
  - 用户说"改用英文回答" → 抽出 `content="用户希望回答使用英文"`
  - 用户说"A 项目取消了,改做 B" → 抽出 `content="用户改做 B 项目"`(episodic)
- 这类表达 memory_type 沿用原偏好类型(改偏好→core,改项目→episodic),importance 与 stability_score 按其**新表达本身**的稳定度评估,不因是"更新型"而降分。
