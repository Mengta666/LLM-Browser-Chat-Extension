...
| `CHAT_COMPACT_SUMMARY_TARGET_TOKENS` | `600` | 摘要生成目标，独立于验收上限；有效值限制在 1 到验收上限之间 |
| `CHAT_COMPACT_SUMMARY_MAX_TOKENS` | `900` | 摘要正文验收上限，不额外叠加容差；完整请求仍受上下文预算保护 |
...
摘要生成目标默认 600 tokens，验收上限默认 900 tokens，两者独立：调大验收上限不会按比例调大生成目标。正文超过生成目标但未超过验收上限时仍可接受；超出验收上限才进入精简。精简目标沿用逐级收紧策略，按独立目标的 100% / 80% / 60% 计算，默认是 600 / 480 / 360 tokens；这些是生成提示，不是截断长度。`CHAT_COMPACT_MAX_OUTPUT_TOKENS` 仍是独立的模型调用输出上限，完整请求预算也不变。

摘要与正式回答分别计时，压缩完成后再开始计算回答阶段的 `CHAT_LLM_TIMEOUT`，但两者仍受整轮上限约束。摘要输出截断、预算配置错误不会无限自动重试；超过摘要验收上限时先精简，仍不合格则保留检查点等待处理。单条新消息或固定提示本身已超额时，无法靠压缩旧历史解决，需缩短输入或修正窗口配置。
